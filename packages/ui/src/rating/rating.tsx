...
function StarSolid({ className }: { className?: string }) {
  return (
    <svg
      viewBox="0 0 576 512"
      className={cn('inline-flex h-[1em] w-[1em] shrink-0 fill-current', className)}
    >
...
export function Rating({
  ...
  return (
    ...
    >
      {showScore && (
        <span className="me-1.5 text-xs font-semibold">
          {score.toFixed(1)}
        </span>
      )}
      {isSm ? (
        <span className="flex items-center text-xs">
          ...
          {count != null && (
            <span className="ms-1 flex">({count})</span>
          )}
        </span>
      ) : (
        <>
            ...
            {[1, 2, 3, 4, 5].map((i) => {
              ...
              return (
                <span className="relative flex" key={i}>
                  <StarSolid
                    className={cn({ 'text-yellow-400': isFull })}
                  />
                  {isHalf && (
